...
def invites_receved_view(request):
	profile = Profile.objects.get(user=request.user)
	qs = Relationship.objectss.invatations_received(profile)
...
class ProfileDetailView(LoginRequiredMixin,DetailView):
	...
	def get_context_data(self, **kwargs):			
		...
		rel_r = Relationship.objectss.filter(sender=profile)
		rel_s = Relationship.objectss.filter(receiver=profile)
		rel_receiver = []
		rel_sender = []
		for item in rel_r:
...
def send_invatation(request):
	if request.method == 'POST':
		...
		rel = Relationship.objectss.create(sender=sender, receiver=receiver, status='send')
